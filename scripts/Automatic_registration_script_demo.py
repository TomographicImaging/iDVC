...
size = np.array(
    [[1180, 1380], [1180, 1380],
     [1300,
      1500]])  # size of the volume in which to calculate the overall shift
p3d_0 = [1280, 1280,
         1400]  # point zero on which to calculate the overall shift
...
if (size[0, 0] <= p3d_0[0] <= size[0, 1]) and (
        size[1, 0] <= p3d_0[1] <= size[1, 1]) and (size[2, 0] <= p3d_0[2] <=
                                                   size[2, 1]) is False:
...
# outputfolder =
#                r'C:\Users\zvm34551\Coding_environment\DATA\DVC_output\image_volumes\Dataset_example2\point1_20231110'
# outputfolder =
#                r'C:\Users\zvm34551\Coding_environment\DATA\DVC_output\image_volumes\Set2\uservolume_analysis_test_code2_20231109'
# outputfolder =
#               r'C:\Users\zvm34551\Coding_environment\DATA\DVC_output\image_volumes\Set3\uservolume_analysis_1_20231005'
...
def importimage(filename):
    ...
    if raw is False:
        ...
        Dim_size = np.array(
            (Nx, Ny, stopz))  # Or read that from your mhd info File
        f = open(path, 'rb')
        if bit == 8:
            image = np.fromfile(f, dtype=np.dtype('>u1'))
        elif bit == 16:
            image = np.fromfile(f, dtype=np.dtype('>u2'))
        image = image[0:Nx * Ny * stopz].reshape(
            Dim_size[2], Dim_size[1],
            Dim_size[0])  # reshape(Dim_size[2],Dim_size[1],Dim_size[0])
...
# -----------------------------------------------------
if pointzero is False:
    centre3d_0 = np.array([
        round(image0.shape[0] / 2),
        round(image0.shape[1] / 2),
        round(image0.shape[2] / 2)
    ])
    p3d_0 = centre3d_0
...
if intermediate_plot is False:
    object = AutomaticRegistration(image0,
                                   image1,
                                   p3d_0,
                                   size,
                                   log_folder = working_directory)
    # run automatic registration
    object.run()
else:
    object = AutomaticRegistrationWithPlotting(intermediate_plot,
                                               filename0,
                                               filename1,
                                               outputfolder,
                                               save,
