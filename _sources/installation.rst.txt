...
3.  Create a new environment and install the software by typing ``conda create --name idvc_environment -c conda-forge -c ccpi idvc=24.1.0`` (or replace ``24.1.0`` with the latest version number).
...
3.	Type ``conda install idvc=24.2.0 -c ccpi -c conda-forge`` (or replace ``24.2.0`` with the latest version number)
